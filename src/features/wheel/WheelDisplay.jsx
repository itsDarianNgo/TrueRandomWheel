...
// Icon Components (Unchanged from Response #66)
...
const SHUFFLE_ANIMATION_TICK_DELAY = 200;

const WheelDisplay = () => {
    // State and Refs (Unchanged from #67)
    ...
    // Item Management & Spin Logic Handlers (Unchanged up to shuffle handlers)
    ...
    const handleDirectPointerPositionChange = useCallback((newPosition) => { if (isDisplaySpinning || isShuffleSequenceAnimating) return; setPointerPosition(newPosition); }, [isDisplaySpinning, isShuffleSequenceAnimating]);
    ...
    // ***** REFINED Shuffle Handlers *****
    const handleQuickShuffle = useCallback(async () => {
        console.log("[handleQuickShuffle] Triggered. Items length:", items.length, "Spinning:", isDisplaySpinning, "Shuffling:", isShuffleSequenceAnimating);
        if (isDisplaySpinning || isShuffleSequenceAnimating || !items || items.length < 2) {
            console.warn("[handleQuickShuffle] Conditions not met. Aborting.");
            return;
        }

        setIsShuffleSequenceAnimating(true);
        console.log("[handleQuickShuffle] isShuffleSequenceAnimating SET TO TRUE");

        const itemsCopy = [...items]; // Create a shallow copy
        console.log("[handleQuickShuffle] Original itemsCopy length:", itemsCopy.length);

        PRNG.shuffleArray(itemsCopy); // Shuffle the copy (assuming it mutates in place)
        console.log("[handleQuickShuffle] itemsCopy shuffled. Length still:", itemsCopy.length);

        setItems(itemsCopy); // Set state with the (mutated) copy
        console.log("[handleQuickShuffle] setItems called with shuffled copy.");

        setShuffleAnimationSeed(s => s + 1);
        console.log("[handleQuickShuffle] Animation seed updated.");

        await new Promise(resolve => setTimeout(resolve, SHUFFLE_ANIMATION_TICK_DELAY));

        setIsShuffleSequenceAnimating(false);
        console.log("[handleQuickShuffle] isShuffleSequenceAnimating SET TO FALSE");
    }, [items, isDisplaySpinning, isShuffleSequenceAnimating]);

    const handleShuffleNTimes = useCallback(async () => {
        console.log("[handleShuffleNTimes] Triggered. Items length:", items.length, "Spinning:", isDisplaySpinning, "Shuffling:", isShuffleSequenceAnimating, "CountInput:", shuffleCountInput);
        if (isDisplaySpinning || isShuffleSequenceAnimating || !items || items.length < 2) {
            console.warn("[handleShuffleNTimes] Conditions not met. Aborting.");
            return;
        }

        setIsShuffleSequenceAnimating(true);
        console.log("[handleShuffleNTimes] isShuffleSequenceAnimating SET TO TRUE");
        ...
        let itemsToShuffle = [...items]; // Start with a fresh copy
        console.log(`[handleShuffleNTimes] Shuffling ${count} times. Initial itemsToShuffle length: ${itemsToShuffle.length}`);

        for (let i = 0; i < count; i++) {
            if (!Array.isArray(itemsToShuffle) || itemsToShuffle.length === 0) { // Defensive check
                console.error(`[handleShuffleNTimes] Loop ${i}: itemsToShuffle is not a valid array or is empty. Aborting further shuffles.`, itemsToShuffle);
                setIsShuffleSequenceAnimating(false);
                return;
            }
            PRNG.shuffleArray(itemsToShuffle); // Mutates itemsToShuffle
            console.log(`[handleShuffleNTimes] Loop ${i}: shuffled. Length: ${itemsToShuffle.length}`);
        }

        setItems(itemsToShuffle); // Set state with the final shuffled copy
        console.log("[handleShuffleNTimes] setItems called with final shuffled items.");

        const animationTickEffectiveDelay = SHUFFLE_ANIMATION_TICK_DELAY;
        for (let i = 0; i < count; i++) {
            setShuffleAnimationSeed(s => s + 1);
            console.log(`[handleShuffleNTimes] Animation tick ${i+1} triggered.`);
            if (i < count - 1) {
                await new Promise(resolve => setTimeout(resolve, animationTickEffectiveDelay));
            } else {
                // Shorter delay after the absolute last tick trigger, just to let React process state
                await new Promise(resolve => setTimeout(resolve, Math.max(50, animationTickEffectiveDelay - 100) ));
            }
        }

        setIsShuffleSequenceAnimating(false);
        console.log("[handleShuffleNTimes] isShuffleSequenceAnimating SET TO FALSE");
    ...
    return (
        <div className="w-full flex flex-col items-center justify-start p-4 sm:p-6 lg:p-8 space-y-8 min-h-screen relative overflow-x-hidden">
            ...
            {/* Winning Banner Modal (unchanged) */}
                ...
                <section className="flex flex-col items-center space-y-4 transition-transform duration-300 ease-in-out w-full h-full flex-grow justify-center" style={{ transform: `translateX(${wheelTranslateX}px)` }}>
                    <div ref={wheelAreaRef} className="bg-slate-800 p-3 sm:p-4 rounded-xl shadow-xl flex items-center justify-center w-full max-w-[90vw] sm:max-w-[80vw] md:max-w-[70vw] lg:max-w-[60vw] xl:max-w-[800px] h-auto aspect-square relative">
                        ...
                        <button
                            ...
                            disabled={isDisplaySpinning || isShuffleSequenceAnimating || items.length < 2}
                            className="absolute top-2 right-2 sm:top-3 sm:right-3 z-10 p-2.5 bg-slate-700/80 hover:bg-slate-600/90 text-slate-100 rounded-full shadow-lg focus:outline-none focus:ring-2 focus:ring-sky-400 focus:ring-offset-2 focus:ring-offset-slate-800 transition-all duration-150 ease-in-out hover:scale-110 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
            ...
            {/* SlideOutPanel - Pass through isShuffleSequenceAnimating to SettingsPanelContent */}
            <SlideOutPanel isOpen={isItemPanelOpen} onClose={() => setIsItemPanelOpen(false)} position="right" title="Wheel Settings & Items" widthClass="w-full sm:w-[420px] md:w-[480px]">
                <SettingsPanelContent
                    isDisplaySpinning={isDisplaySpinning || isShuffleSequenceAnimating} // Pass combined lock
