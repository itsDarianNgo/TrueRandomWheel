...
import WheelControls from './WheelControls'; // WheelControls will be simpler now
import AddItemForm from '../itemManagement/AddItemForm';
import CurrentItemsList from '../itemManagement/CurrentItemsList';
import SlideOutPanel from '../../components/SlideOutPanel'; // Using your version

// Icons (ensure these are defined as in Response #45 or your working version)
const IconSettings = ({ className = "w-6 h-6" }) => ( /* ... SVG ... */ <svg xmlns="http://www.w3.org/2000/svg" className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" /><path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg>);
const IconClosePanel = ({ className = "w-6 h-6" }) => ( /* ... SVG ... */ <svg xmlns="http://www.w3.org/2000/svg" className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>);
const IconWheelOutlineDisplay = () => ( /* ... SVG ... */ <svg xmlns="http://www.w3.org/2000/svg" className="h-16 w-16 text-slate-600 mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1"><path strokeLinecap="round" strokeLinejoin="round" d="M12 21a9 9 0 100-18 9 9 0 000 18z" /><path strokeLinecap="round" strokeLinejoin="round" d="M12 3v3m0 12v3m9-9h-3M3 12H0M12 12l5.25 5.25M12 12l-5.25 5.25M12 12l5.25-5.25M12 12l-5.25-5.25" /></svg>);

// Re-importing pointer icons here as they are used in the settings panel's segmented control
...
const pointerIconsMapWheelDisplay = { // Renamed to avoid conflict if WheelControls also had one locally
...
const WheelDisplay = () => {
    ...
    // handleSpinClick, handleSpinStart, handleSpinEnd, canSpin
    ...
    }, [pointerPosition]); // pointerPosition is a dependency here

    const currentWheelSize = isItemPanelOpen ? closedPanelWheelRenderSize * WHEEL_SCALE_WHEN_PANEL_OPEN : closedPanelWheelRenderSize;
    // Note: The above is for the visual scaled size. The canvas itself renders at `closedPanelWheelRenderSize`.
    ...
    return (
        <div className="w-full flex flex-col items-center justify-start p-4 sm:p-6 lg:p-8 space-y-8 min-h-screen relative overflow-x-hidden">
            ...
            <div className="w-full max-w-7xl mx-auto flex flex-col items-center justify-center pt-10 xl:pt-0">
                <section className="flex flex-col items-center space-y-6 transition-transform duration-300 ease-in-out" style={{ transform: isItemPanelOpen ? `translateX(-${WHEEL_SECTION_TRANSLATE_X_OFFSET}px)` : 'translateX(0px)', width: `${closedPanelWheelRenderSize + 64}px`, maxWidth: '100%' }}>
                    <div className="bg-slate-800 p-4 sm:p-6 rounded-xl shadow-xl">
                        <div className="relative mx-auto origin-center transition-transform duration-300 ease-in-out" style={{ width: `${closedPanelWheelRenderSize}px`, height: `${closedPanelWheelRenderSize}px`, transform: isItemPanelOpen ? `scale(${WHEEL_SCALE_WHEN_PANEL_OPEN})` : 'scale(1)' }}>
                            ...
                            {items.length === 0 && ( /* ... Unchanged Empty State Overlay ... */ <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-800/80 backdrop-blur-sm rounded-full pointer-events-none"><IconWheelOutlineDisplay /><p className="text-lg font-medium text-slate-300 p-4 text-center">Your wheel is empty!</p><p className="text-sm text-slate-400">Use the form to add items.</p></div>)}
                        </div>
                    </div>
                    <div className="w-full origin-top transition-transform duration-300 ease-in-out" style={{ maxWidth: `${closedPanelWheelRenderSize}px`, transform: isItemPanelOpen ? `scale(${WHEEL_SCALE_WHEN_PANEL_OPEN})` : 'scale(1)' }}>
                        <WheelControls
                            onSpinClick={handleSpinClick}
                            isSpinning={isDisplaySpinning}
                            canSpin={canSpin}
                            currentPointerPosition={pointerPosition}
                            onCyclePointerPosition={handleCyclePointerPosition} // Pass new handler
                        />
                    </div>
                </section>
            </div>
            ...
            <SlideOutPanel isOpen={isItemPanelOpen} onClose={() => setIsItemPanelOpen(false)} position="right" title="Wheel Settings & Items" widthClass="w-full sm:w-[420px] md:w-[480px]">
                <div className="flex flex-col space-y-8 p-1">
                    {/* Section for Pointer Position in Panel */}
                    <section className="p-4 bg-slate-700/50 rounded-lg">
                        <h4 className="text-md font-semibold text-sky-300 mb-3 text-center">Pointer Position</h4>
                        <div role="group" aria-labelledby="pointer-position-group-label-panel" className="flex w-full rounded-lg shadow-sm border border-slate-600 overflow-hidden">
                            {POINTER_POSITIONS_ORDER.map((positionKey, index) => {
                                ...
                                return (
                                    <button
                                        ...
                                        onClick={() => handleDirectPointerPositionChange(positionKey)} // Use direct change handler
                                        className={`flex-1 flex flex-col items-center justify-center p-3 transition-colors duration-150 ease-in-out focus:outline-none focus:z-10 focus:ring-2 focus:ring-offset-0 focus:ring-sky-400 ${segmentClasses} ${isActive ? 'bg-sky-600 text-white shadow-inner' : 'bg-slate-700 text-slate-300 hover:bg-slate-600'}`}
