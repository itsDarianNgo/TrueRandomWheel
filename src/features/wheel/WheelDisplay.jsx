...
// ... (All Icon components, pointerIconsMapWheelDisplay, POINTER_POSITIONS_ORDER as in #47) ...
...
const WheelDisplay = () => {
    ...
    // All handlers (handleAddItem, handleRemoveItemGroup, handleSpinClick, handleDirectPointerPositionChange,
    // handleCyclePointerPosition, handleSpinStart, handleSpinEnd) are UNCHANGED from Response #48
    // useEffect for banner cleanup (unchanged)
    ...
    return (
        <div className="w-full flex flex-col items-center justify-start p-4 sm:p-6 lg:p-8 space-y-8 min-h-screen relative overflow-x-hidden">
            ...
            <SlideOutPanel isOpen={isItemPanelOpen} onClose={() => setIsItemPanelOpen(false)} position="right" title="Wheel Settings & Items" widthClass="w-full sm:w-[420px] md:w-[480px]">
                <div className={`flex flex-col space-y-8 p-1 ${isDisplaySpinning ? 'opacity-60 pointer-events-none' : ''}`}> {/* MODIFICATION: Disable panel content interaction */}
                    <section className="p-4 bg-slate-700/50 rounded-lg">
                        ...
                        <div role="group" aria-labelledby="pointer-position-group-label-panel" className={`flex w-full rounded-lg shadow-sm border border-slate-600 overflow-hidden ${isDisplaySpinning ? 'opacity-50 cursor-not-allowed' : ''}`}> {/* Inner control group disabling */}
                            {POINTER_POSITIONS_ORDER.map((positionKey, index) => { /* ... (button rendering as in #47, but with disabled={isDisplaySpinning}) ... */
                                ...
                                return (<button key={positionKey} type="button" onClick={() => handleDirectPointerPositionChange(positionKey)} disabled={isDisplaySpinning} className={`flex-1 flex flex-col items-center justify-center p-3 transition-colors duration-150 ease-in-out focus:outline-none focus:z-10 focus:ring-2 focus:ring-offset-0 focus:ring-sky-400 ${segmentClasses} ${isActive ? 'bg-sky-600 text-white shadow-inner' : 'bg-slate-700 text-slate-300 hover:bg-slate-600'} ${isDisplaySpinning ? 'pointer-events-none hover:bg-slate-700' : ''}`} aria-pressed={isActive} aria-label={`Set pointer to ${label}`}><IconComponent className={`w-5 h-5 mb-1 ${isActive ? 'text-white' : 'text-slate-400'}`} /><span className="text-xs font-semibold">{label}</span></button>);
                            })}
                        </div>
                    </section>
                    {/* ***** MODIFICATION: Pass isSpinning prop ***** */}
                    <AddItemForm onAddItem={handleAddItem} isSpinning={isDisplaySpinning} />
                    <CurrentItemsList items={items} onRemoveItemGroup={handleRemoveItemGroup} isSpinning={isDisplaySpinning} className="w-full" />
                </div>
            </SlideOutPanel>
