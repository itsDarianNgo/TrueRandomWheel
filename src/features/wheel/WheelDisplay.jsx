...
const IconClosePanel = ({ className = "w-6 h-6" }) => (
    <svg xmlns="http://www.w3.org/2000/svg" className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
...
const WheelPage = () => {
    // ... (state, refs, constants, useEffects, callbacks - same as Response #46) ...
    ...
    useEffect(() => { const pageElement = pageRootRef.current; if (!pageElement) return; if (pageBackgroundImageUrl && /^(https?:\/\/|data:image\/)/i.test(pageBackgroundImageUrl)) { pageElement.style.backgroundImage = `url('${pageBackgroundImageUrl}')`; pageElement.style.backgroundSize = 'cover'; pageElement.style.backgroundPosition = 'center'; pageElement.style.backgroundRepeat = 'no-repeat'; pageElement.style.backgroundColor = 'transparent'; } else { pageElement.style.backgroundImage = ''; pageElement.style.backgroundColor = ''; } }, [pageBackgroundImageUrl]);
    ...
    return (
        <div ref={pageRootRef} className="w-full flex flex-col items-center justify-start p-4 sm:p-6 lg:p-8 space-y-8 min-h-screen relative overflow-x-hidden transition-all duration-500 ease-in-out">
            {/* Settings Toggle Button - Uses IconSettings and IconClosePanel */}
            <button
            ...
            {/* Winning Banner Modal (same JSX as Response #46) */}
            ...
            <div className="w-full mx-auto flex flex-col items-center justify-center flex-grow relative">
                <section className="flex flex-col items-center space-y-4 transition-transform duration-300 ease-in-out w-full h-full flex-grow justify-center" style={{ transform: `translateX(${wheelTranslateXVal}px)` }}>
                    <div ref={wheelAreaRef} className={`p-3 sm:p-4 rounded-xl shadow-2xl flex items-center justify-center w-full max-w-[90vw] sm:max-w-[80vw] md:max-w-[70vw] lg:max-w-[60vw] xl:max-w-[800px] h-auto aspect-square relative ${(pageBackgroundImageUrl || wheelStatus !== 'idle') ? 'bg-transparent' : 'bg-slate-800/70'}`} >
                        <div className="relative origin-center transition-transform duration-300 ease-in-out" style={{ width: `${canvasDimensions.width}px`, height: `${canvasDimensions.height}px`, transform: `scale(${visualWheelScale})`}}>
                            ...
                            {items.length === 0 && !overallIsBusy && ( <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-800/90 backdrop-blur-sm rounded-full pointer-events-none p-5"> <IconWheelOutlineDisplay /> <p className="text-lg font-medium text-slate-300 mt-3 text-center">Your wheel is empty!</p> <p className="text-sm text-slate-400 text-center">Use the settings panel to add items.</p> </div> )}
                        </div>
                        {/* Quick Shuffle Button - Uses IconShuffle */}
                        <button
