...
// ... (all imports, icons, constants, state, handlers as in Response #56) ...
...
// All Icon components, pointerIconsMapWheelDisplay, POINTER_POSITIONS_ORDER as in Response #47/56
...
// New Info Icon
const IconInformationCircle = ({ className = "w-4 h-4" }) => (
...
const WheelDisplay = () => {
    // State and handlers (all unchanged from Response #56)
    ...
    const handleSpinEnd = useCallback((spunItem, errorInfo) => { setIsDisplaySpinning(false); if (errorInfo) { console.error("Spin ended with error:", errorInfo.error); setWinningItem(null); return; } if (spunItem) { setWinningItem(spunItem); setShowWinningBanner(true); bannerTimeoutRef.current = setTimeout(() => setShowWinningBanner(false), 6000); if (removeOnHit) { setItems(prevItems => prevItems.filter(item => item.id !== spunItem.id)); } } else { setWinningItem(null); } }, [removeOnHit]);
    ...
    return (
        <div className="w-full flex flex-col items-center justify-start p-4 sm:p-6 lg:p-8 space-y-8 min-h-screen relative overflow-x-hidden">
            ...
            {/* Winning Banner (Unchanged from #48) */}
            ...
            {/* SlideOutPanel Content - "Spin Behavior" section MODIFIED */}
            <SlideOutPanel isOpen={isItemPanelOpen} onClose={() => setIsItemPanelOpen(false)} position="right" title="Wheel Settings & Items" widthClass="w-full sm:w-[420px] md:w-[480px]">
                <div className={`flex flex-col space-y-6 p-2 sm:p-4 ${isDisplaySpinning ? 'opacity-60 pointer-events-none' : ''}`}> {/* Adjusted padding & space-y */}

                    {/* Pointer Position Settings Section (Unchanged from #47) */}
                    <section className="p-4 bg-slate-700/60 rounded-lg shadow"> {/* Added shadow for depth */}
                    ...
                    {/* ***** REFINED: Spin Behavior Settings Section ***** */}
                    <section className="p-4 bg-slate-700/60 rounded-lg shadow">
                        ...
                        {/* Using flex, label on left, toggle on right */}
                        <div className={`flex items-center justify-between p-3 bg-slate-700 rounded-md transition-opacity ${isDisplaySpinning ? 'opacity-50' : ''}`}>
                            <div className="flex items-center space-x-2">
                                <label htmlFor="removeOnHitToggle" className="text-sm text-slate-200 select-none cursor-pointer flex-grow">
                                    Auto-Remove Winner
                                </label>
                                <span
                                    className="cursor-help text-slate-400 hover:text-sky-400 transition-colors"
                                    title="If enabled, the winning item segment will be removed from the wheel after each spin."
                                >
                            <IconInformationCircle className="w-4 h-4"/>
                        </span>
                            </div>
                            <button
                                type="button"
                                id="removeOnHitToggle"
                                role="switch"
                                aria-checked={removeOnHit}
                                onClick={() => { if (!isDisplaySpinning) setRemoveOnHit(prev => !prev); }} // Prevent toggle if spinning
                                disabled={isDisplaySpinning} // Explicitly disable
                                className={`relative inline-flex items-center h-6 rounded-full w-11 transition-colors focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2 focus:ring-offset-slate-700 ${isDisplaySpinning ? 'cursor-not-allowed' : 'cursor-pointer'} ${removeOnHit ? 'bg-sky-500' : 'bg-slate-600'}`}
                            >
                    ...
                    {/* Item Management (unchanged from #49) */}
            ...
            {/* Debug Info Area (unchanged from #48) */}
            <div className="fixed bottom-0 left-0 right-0 p-3 bg-slate-900/80 backdrop-blur-sm text-xs text-slate-400 text-center z-[50]">
                <span>...Debug Info... | Panel: {isItemPanelOpen.toString()} | WheelScale: {visualWheelScale} | WheelX: {wheelTranslateX}px | isSpinning: {isDisplaySpinning.toString()} | RemoveOnHit: {removeOnHit.toString()}</span>
            </div>
        </div>
    );
};
