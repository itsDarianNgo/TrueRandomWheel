...
// Default Values & Helper Functions (isColorLight) - Unchanged from Response #50
...
// Colors for shuffle tick animation flash
const SHUFFLE_FLASH_COLORS = ['#FCA5A5', '#FCD34D', '#86EFAC', '#93C5FD', '#C4B5FD']; // Light, distinct colors
const SHUFFLE_TICK_DURATION = 150; // ms for one flash animation
...
const WheelCanvas = forwardRef(({
                                    ...
                                    onSpinEnd = () => {},
                                    onWheelClick = () => {},
                                    shuffleAnimationTrigger = 0, // ***** NEW PROP *****
    ...
    // ***** NEW STATE for Shuffle Tick Animation *****
    const [isShufflingTick, setIsShufflingTick] = useState(false);
    const [shuffleTickColorOffset, setShuffleTickColorOffset] = useState(0); // To cycle flash colors

    // getPointerTargetAngle, drawSegmentText, drawSegment (modified), drawPointer, drawWheel (modified)
    ...
    const drawSegmentText = useCallback((ctx, item, textX, textY, maxTextWidth, radius, numSegments, itemActualColor) => { /* ... Unchanged from #50 ... */ const baseFontSize = radius / (numSegments === 1 ? 6 : (numSegments > 10 ? 12 : 10)); const dynamicFontSize = Math.max(12, Math.min(numSegments === 1 ? 40 : (numSegments > 6 ? 22 : 28), baseFontSize)); ctx.font = `600 ${dynamicFontSize}px ${fontFamily}`; ctx.fillStyle = overrideTextColor || (isColorLight(itemActualColor) ? DEFAULT_TEXT_COLOR_DARK : DEFAULT_TEXT_COLOR_LIGHT); ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; let displayText = item.name; if (ctx.measureText(displayText).width > maxTextWidth) { for (let i = displayText.length - 1; i > 0; i--) { displayText = item.name.substring(0, i) + '...'; if (ctx.measureText(displayText).width <= maxTextWidth) break; } if (ctx.measureText(displayText).width > maxTextWidth) { displayText = item.name.substring(0,1); if (ctx.measureText(displayText).width > maxTextWidth && item.name.length > 0) displayText = ""; } } ctx.save(); ctx.translate(textX, textY); ctx.fillText(displayText, 0, 0); ctx.restore(); }, [fontFamily, overrideTextColor]);

    // MODIFIED drawSegment to use temporary flash colors during shuffle tick
    const drawSegment = useCallback((ctx, item, index, numSegments, centerX, centerY, radius) => {
        let itemActualColor = item.color || defaultSegmentColors[index % defaultSegmentColors.length];
        // ***** If shuffling tick, use flash color *****
        if (isShufflingTick) {
            itemActualColor = SHUFFLE_FLASH_COLORS[(index + shuffleTickColorOffset) % SHUFFLE_FLASH_COLORS.length];
        }
        ctx.fillStyle = itemActualColor;

        let currentSegmentStrokeStyle; // Dynamic stroke logic (unchanged from #50)
        if (overrideSegmentStrokeColor && overrideSegmentStrokeColor !== 'transparent') { currentSegmentStrokeStyle = overrideSegmentStrokeColor; } else { currentSegmentStrokeStyle = isColorLight(itemActualColor) ? DEFAULT_DARK_SEGMENT_STROKE_COLOR : DEFAULT_LIGHT_SEGMENT_STROKE_COLOR; }

        if (numSegments === 1) { /* ... Drawing logic for 1 segment (unchanged from #50, uses itemActualColor) ... */ ctx.beginPath(); ctx.arc(centerX, centerY, radius, 0, 2 * Math.PI); ctx.closePath(); ctx.fill(); if (segmentStrokeWidth > 0) { ctx.strokeStyle = currentSegmentStrokeStyle; ctx.lineWidth = segmentStrokeWidth; ctx.stroke(); } const maxTextWidthForSingle = radius * 1.6; drawSegmentText(ctx, item, centerX, centerY, maxTextWidthForSingle, radius, numSegments, itemActualColor); } else { /* ... Drawing logic for >1 segments (unchanged from #50, uses itemActualColor) ... */ const segmentAngle = (2 * Math.PI) / numSegments; const startAngle = index * segmentAngle; const endAngle = startAngle + segmentAngle; ctx.beginPath(); ctx.moveTo(centerX, centerY); ctx.arc(centerX, centerY, radius, startAngle, endAngle); ctx.closePath(); ctx.fill(); if (segmentStrokeWidth > 0) { ctx.strokeStyle = currentSegmentStrokeStyle; ctx.lineWidth = segmentStrokeWidth; ctx.stroke(); } const textRadius = radius * 0.65; const textAngle = startAngle + segmentAngle / 2; const textX = centerX + textRadius * Math.cos(textAngle); const textY = centerY + textRadius * Math.sin(textAngle); const maxTextWidthForSegment = (2 * textRadius * Math.sin(segmentAngle / 2)) * 0.80; drawSegmentText(ctx, item, textX, textY, maxTextWidthForSegment, radius, numSegments, itemActualColor); }
    }, [fontFamily, defaultSegmentColors, segmentStrokeWidth, overrideTextColor, overrideSegmentStrokeColor, drawSegmentText, isShufflingTick, shuffleTickColorOffset]); // Added animation states

    const drawPointer = useCallback((ctx, centerX, centerY, radius) => { /* ... Unchanged ... */ const pointerAngle = getPointerTargetAngle(); const pointerLength = radius * 0.18; const pointerHalfWidthAtBase = radius * 0.05; ctx.fillStyle = pointerColor; ctx.beginPath(); const tipX = centerX + (radius + segmentStrokeWidth) * Math.cos(pointerAngle); const tipY = centerY + (radius + segmentStrokeWidth) * Math.sin(pointerAngle); const baseCenterX = centerX + (radius + segmentStrokeWidth + pointerLength) * Math.cos(pointerAngle); const baseCenterY = centerY + (radius + segmentStrokeWidth + pointerLength) * Math.sin(pointerAngle); const perpAngle1 = pointerAngle + Math.PI / 2; const perpAngle2 = pointerAngle - Math.PI / 2; const baseX1 = baseCenterX + pointerHalfWidthAtBase * Math.cos(perpAngle1); const baseY1 = baseCenterY + pointerHalfWidthAtBase * Math.sin(perpAngle1); const baseX2 = baseCenterX + pointerHalfWidthAtBase * Math.cos(perpAngle2); const baseY2 = baseCenterY + pointerHalfWidthAtBase * Math.sin(perpAngle2); ctx.moveTo(tipX, tipY); ctx.lineTo(baseX1, baseY1); ctx.lineTo(baseX2, baseY2); ctx.closePath(); ctx.fill(); }, [getPointerTargetAngle, pointerColor, segmentStrokeWidth]);

    // drawWheel now depends on isShufflingTick and shuffleTickColorOffset for re-draws during animation
    const drawWheel = useCallback(() => { /* ... Most logic unchanged, but drawSegment now uses animation states ... */ const canvas = canvasRef.current; if (!canvas) return; const ctx = canvas.getContext('2d'); const numSegments = items.length; const centerX = width / 2; const centerY = height / 2; const wheelRadius = Math.min(centerX, centerY) * 0.90; ctx.clearRect(0, 0, width, height); if (numSegments === 0 && !isShufflingTick) { ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillStyle = '#6B7280'; ctx.font = `500 16px ${fontFamily}`; ctx.fillText("Wheel is currently empty.", centerX, centerY); return; } ctx.save(); ctx.translate(centerX, centerY); ctx.rotate(currentWheelRotation); items.forEach((item, index) => { drawSegment(ctx, item, index, numSegments, 0, 0, wheelRadius); }); ctx.restore(); drawPointer(ctx, centerX, centerY, wheelRadius); }, [items, width, height, currentWheelRotation, fontFamily, drawSegment, drawPointer, isShufflingTick]); // isShufflingTick added, drawSegment has its own deps including shuffleTickColorOffset

    useEffect(() => { drawWheel(); }, [drawWheel]); // This will re-run if drawWheel itself changes due to its deps.

    // useEffect for item changes (unchanged)
    useEffect(() => { if (!internalIsSpinning && !isShufflingTick) { setCurrentWheelRotation(prev => (prev % (2 * Math.PI) + (2 * Math.PI)) % (2 * Math.PI)); } }, [items, internalIsSpinning, isShufflingTick]); // Added isShufflingTick

    // ***** NEW useEffect for Shuffle Tick Animation Trigger *****
    const shuffleAnimationTimerRef = useRef(null);
    useEffect(() => {
        if (shuffleAnimationTrigger === 0) return; // Ignore initial value
        if (internalIsSpinning || items.length < 2) return; // Don't animate if spinning or too few items

        console.log("WheelCanvas: Shuffle animation tick triggered by seed:", shuffleAnimationTrigger);
        setIsShufflingTick(true);
        setShuffleTickColorOffset(prev => (prev + 1) % SHUFFLE_FLASH_COLORS.length); // Cycle colors for variety
        ...
        shuffleAnimationTimerRef.current = setTimeout(() => {
            setIsShufflingTick(false);
            // drawWheel(); // Implicitly called by state change if drawWheel depends on isShufflingTick
            console.log("WheelCanvas: Shuffle animation tick ended.");
        }, SHUFFLE_TICK_DURATION);

        return () => {
            if (shuffleAnimationTimerRef.current) clearTimeout(shuffleAnimationTimerRef.current);
        };
    }, [shuffleAnimationTrigger, internalIsSpinning, items]);


    // spin method (Unchanged from Response #50/66)
    ...
    return ( <canvas ref={canvasRef} width={width} height={height} onClick={onWheelClick} className={`wheel-canvas ${canvasClassName} rounded-full ${items.length > 0 && !internalIsSpinning && !isShufflingTick ? 'cursor-pointer hover:opacity-90 active:opacity-80' : 'cursor-default'} transition-opacity`} role="button" tabIndex={items.length > 0 && !internalIsSpinning && !isShufflingTick ? 0 : -1} aria-label="Spin the wheel by clicking it" /> );
});
