...
// PRNG is no longer used directly in WheelCanvas for selecting winner
...
const SHUFFLE_SPIN_TICK_DURATION = 250;
const SHUFFLE_SPIN_ROTATIONS = 1.5;
const SHUFFLE_SPIN_ANGULAR_VELOCITY = (SHUFFLE_SPIN_ROTATIONS * 2 * Math.PI) / (SHUFFLE_SPIN_TICK_DURATION / 1000);

const WheelCanvas = forwardRef(({
                                    items = [],
                                    pointerPosition = 'right', // From Redux via WheelPage
                                    onSpinStart = () => {},
                                    onSpinEnd = () => {},
                                    onWheelClick = () => {}, // Typically triggers spinThunk in parent

                                    // shuffleAnimationTrigger prop might be deprecated if shuffle animation simplified
                                    // For now, keeping its logic if parent still uses it.
                                    // If shuffle is just reordering items, WheelCanvas just re-renders.
                                    // Let's assume it's still used for a visual "tick" if parent sends it.
                                    shuffleAnimationTrigger = 0,

                                    minSpins = DEFAULT_MIN_SPINS,         // From Redux via WheelPage
                                    spinDuration = DEFAULT_SPIN_DURATION, // From Redux via WheelPage

                                    width = 480,
                                    height = 480,
                                    pointerColor = DEFAULT_POINTER_COLOR, // Could be configurable too
    ...
    const drawWheel = useCallback(() => {
        ...
        if (numSegments === 0 && wheelStatus === 'idle' && !internalIsSpinning && !isShuffleSpinAnimating) {
            ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillStyle = '#6B7280'; // Slate-500
            ctx.font = `500 ${Math.max(14, wheelRadius / 15)}px ${fontFamily}`; // Responsive font
        ...
        // Optional: Generic "busy" overlay if wheelStatus indicates
        if (wheelStatus === 'shuffle_animating' && !isShuffleSpinAnimating) { // If main shuffle is happening, not just a tick
            ctx.fillStyle = 'rgba(0, 0, 0, 0.3)';
            ctx.fillRect(0, 0, width, height);
            ctx.fillStyle = 'white';
            ctx.font = `bold ${Math.max(16, wheelRadius / 12)}px ${fontFamily}`;
    ...
    }, [items, width, height, currentWheelRotation, fontFamily, drawSegment, drawPointer, internalIsSpinning, isShuffleSpinAnimating, wheelStatus]); // Added wheelStatus
    ...
    // New Targeted Spin Logic
    const performTargetedSpin = useCallback((targetItemToWin, minSpinsOverride, spinDurationOverride) => {
        if (internalIsSpinning || isShuffleSpinAnimating || items.length === 0 || !targetItemToWin) {
            if (!targetItemToWin) console.error("WheelCanvas: spinToTarget called without a targetItemToWin.");
            else if (items.length === 0) console.error("WheelCanvas: spinToTarget called with no items on the wheel.");
            else if (internalIsSpinning) console.warn("WheelCanvas: spinToTarget called while already spinning.");
            else if (isShuffleSpinAnimating) console.warn("WheelCanvas: spinToTarget called while shuffle animating.");
        ...
        onSpinStart(targetItemToWin); // Notify parent, pass the target item
        ...
        if (winningItemIndex === -1) {
            console.error("WheelCanvas: Target item ID not found in current wheel items:", targetItemToWin.id);
        ...
        const performAnimationFrame = () => {
            ...
            // Ensure progress doesn't exceed 1 due to timing issues
            ...
            if (progress < 1) {
                ...
                setCurrentWheelRotation(finalWheelRotation);
                setInternalIsSpinning(false);
                animationFrameIdRef.current = null;
                onSpinEnd(targetItemToWin, null); // Spin ended successfully, report the item that was targeted
            }
        };
        animationFrameIdRef.current = requestAnimationFrame(performAnimationFrame);
    }, [items, internalIsSpinning, isShuffleSpinAnimating, currentWheelRotation, getPointerTargetAngle, onSpinStart, onSpinEnd, minSpins, spinDuration]);


    // Shuffle Tick Animation Logic (kept if shuffleAnimationTrigger prop is still used by parent)
    const animateShuffleTick = useCallback(() => {
        // This function is called if parent wants a specific visual tick.
        // It's distinct from just re-rendering due to `items` prop change.
        if (internalIsSpinning || items.length < 2) { // Don't shuffle tick if prize spinning or not enough items
            if(isShuffleSpinAnimating) setIsShuffleSpinAnimating(false);
            return;
        }

        setIsShuffleSpinAnimating(true);
        shuffleSpinAnimationStartTimeRef.current = performance.now();
        shuffleSpinInitialRotationRef.current = currentWheelRotation;
        ...
        const animationTick = (timestamp) => {
            ...
            if (elapsed < SHUFFLE_SPIN_TICK_DURATION) {
                const rotationThisFrame = SHUFFLE_SPIN_ANGULAR_VELOCITY * (elapsed / 1000);
                ...
                // Normalize rotation at end of tick to avoid large numbers if not spinning again soon
                setCurrentWheelRotation(prev => (prev % (2 * Math.PI) + (2 * Math.PI)) % (2 * Math.PI));
                setIsShuffleSpinAnimating(false);
                // Parent (WheelPage) is responsible for setting overall wheelStatus back to 'idle'
                // after its shuffle sequence (multiple ticks) is complete.
            }
        };
        animationFrameIdRef.current = requestAnimationFrame(animationTick);
    }, [items, internalIsSpinning, currentWheelRotation]); // Removed isShuffleSpinAnimating from deps to allow re-trigger

    useEffect(() => {
        // This effect reacts to shuffleAnimationTrigger from parent.
        // If parent increments this, we run a visual tick.
        if (shuffleAnimationTrigger > 0 && !internalIsSpinning && items.length >=2) { // Check internalIsSpinning
            // console.log("WheelCanvas: Shuffle animation tick triggered by seed:", shuffleAnimationTrigger);
            animateShuffleTick();
        }
        // No cleanup needed here that would stop an ongoing prize spin.
        // Cleanup for animationFrameIdRef.current is in general cleanup.
    }, [shuffleAnimationTrigger, items.length, internalIsSpinning, animateShuffleTick]);
    ...
    useImperativeHandle(ref, () => ({
        ...
        // Expose shuffle tick animation if parent needs to trigger it manually
        // This is used by WheelPage's simplified Phase 1 shuffle logic.
        triggerShuffleAnim: () => {
            if(!isShuffleSpinAnimating && !internalIsSpinning && items.length >=2 ) { // Prevent overlapping shuffle anims
                animateShuffleTick();
            }
        }
    }), [performTargetedSpin, animateShuffleTick, isShuffleSpinAnimating, internalIsSpinning, items.length]);

    useEffect(() => { // General cleanup for any animation frame
        return () => {
    ...
    const canBeClicked = !internalIsSpinning && !isShuffleSpinAnimating && items.length > 0 && wheelStatus === 'idle';
    ...
    return (
        <canvas
            ...
            width={width}
            height={height}
            onClick={canBeClicked ? onWheelClick : undefined} // onWheelClick now comes from parent (WheelPage)
            className={`wheel-canvas ${canvasClassName} rounded-full ${canBeClicked ? 'cursor-pointer hover:opacity-90 active:opacity-80' : 'cursor-default'} transition-opacity duration-150 ease-in-out`}
            role={canBeClicked ? "button" : undefined}
            tabIndex={canBeClicked ? 0 : -1}
            aria-label={canBeClicked ? "Spin the wheel" : "Wheel is busy or empty"}
        />
    );
});
...
WheelCanvas.propTypes = {
    items: PropTypes.arrayOf(PropTypes.shape({
        ...
        sourceGroup: PropTypes.string, // Optional, but useful for item management
    })).isRequired,
    ...
    shuffleAnimationTrigger: PropTypes.number, // If used for visual ticks
    ...
    wheelStatus: PropTypes.string, // e.g., 'idle', 'spinning', 'shuffle_animating'
};
