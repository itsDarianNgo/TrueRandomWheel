...
const WheelCanvas = forwardRef(({
    ...
    useEffect(() => { /* ... image loading logic from Response #69 (handles data URLs) ... */ if (wheelSurfaceImageUrl && /^(https?:\/\/|data:image\/)/i.test(wheelSurfaceImageUrl)) { setSurfaceImageStatus('loading'); const img = new Image(); img.onload = () => { setSurfaceImage(img); setSurfaceImageStatus('loaded'); }; img.onerror = () => { console.warn("WheelCanvas: Failed to load wheel surface image:", wheelSurfaceImageUrl); setSurfaceImage(null); setSurfaceImageStatus('error'); }; img.src = wheelSurfaceImageUrl; return () => { img.onload = null; img.onerror = null; }; } else { setSurfaceImage(null); setSurfaceImageStatus('idle'); if (wheelSurfaceImageUrl) { console.warn("WheelCanvas: Invalid wheel surface image URL provided:", wheelSurfaceImageUrl); } } }, [wheelSurfaceImageUrl]);

    // MODIFIED useEffect for Text Pre-calculation (with MIN_READABLE_FONT_SIZE_PX)
    useEffect(() => {
        ...
        const newProcessedItems = items.map((item, index) => {
            ...
            if (numSegments === 1) {
                ...
                maxTextLengthPx = currentWheelRadius * 1.6;
                ...
                const textInnerRadiusFactor = 0.25;
            ...
            // ***** Optimization: Check font size before measuring/truncating *****
            ...
            ctx.font = fontSizeToUse; // Set font for measurement IF displayText is not empty
            let textWidth = 0;
            if (displayText) { // Only measure if there's text to measure
            ...
            } else { // If displayText was cleared due to small font, width is 0
                textWidth = 0;
            }

            if (textLayoutProps.type === 'radial' && displayText && textWidth < maxTextLengthPx) { // Centering logic if text exists
    ...
    const drawWheel = useCallback(() => {
        ...
        if (numSegments > 0) {
            ...
            processedTextItems.forEach((processedItem) => {
                ...
                // ***** Optimization: Skip drawing if displayText is empty *****
                ...
                if (textLayoutProps.type === 'horizontal-center') {
                    ...
                    ctx.fillText(displayText, 0, 0, textLayoutProps.maxWidth);
                } else if (textLayoutProps.type === 'radial') {
                    ctx.save();
                    ctx.rotate(textLayoutProps.angle);
            ...
            ctx.restore();
        } else if (wheelStatus === 'idle' && !internalIsSpinning) { /* ... same empty state ... */ ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillStyle = '#6B7280'; ctx.font = `500 ${Math.max(14, currentWheelRadius / 15)}px ${fontFamily}`; ctx.fillText("Wheel is empty.", currentCenterX, currentCenterY - 10); ctx.font = `400 ${Math.max(12, currentWheelRadius / 20)}px ${fontFamily}`; ctx.fillText("Add items in settings.", currentCenterX, currentCenterY + 10); }

        if (wheelStatus === 'shuffle_animating') { /* ... same shuffle overlay ... */ ctx.fillStyle = 'rgba(40, 48, 64, 0.6)'; ctx.fillRect(0, 0, currentWidth, currentHeight); ctx.fillStyle = 'rgba(226, 232, 240, 0.9)'; ctx.font = `600 ${Math.max(16, currentWheelRadius / 12)}px ${fontFamily}`; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText('Shuffling...', currentCenterX, currentCenterY); }
        drawPointer(ctx);

    }, [items, currentWidth, currentHeight, currentCenterX, currentCenterY, currentWheelRadius, currentWheelRotation, fontFamily, drawSegmentBase, drawPointer, internalIsSpinning, wheelStatus, surfaceImage, surfaceImageStatus, processedTextItems, segmentOpacity]); // Removed pointerColor from deps of drawWheel as drawPointer handles it.
...
WheelCanvas.propTypes = { /* ... same as Response #69 ... */
