...
import { HexColorPicker, HexColorInput } from 'react-colorful'; // Ensure HexColorPicker is imported
...
const SettingsPanelContent = ({ onShuffleNTimes }) => {
    ...
    // Sync temporaryPointerColor with Redux state when modal is closed or Redux state changes
    useEffect(() => {
        // If modal is not open, ensure temporary color reflects the true current color.
        // Also, when currentCustomPointerColor changes (e.g., on initial load from localStorage),
        // update temporaryPointerColor if modal isn't open.
        if (!isPointerColorModalOpen) {
            setTemporaryPointerColor(currentCustomPointerColor);
        }
    }, [currentCustomPointerColor, isPointerColorModalOpen]);
    ...
    const handleShuffleCountInputChange = (e) => { /* ... unchanged ... */ const val = e.target.value; setLocalShuffleCountInput(val); const count = parseInt(val, 10); if (!isNaN(count) && count >= 1) { dispatch(setReduxShuffleCount(count)); } else if (val === "") { dispatch(setReduxShuffleCount(1)); } };
    ...
    const handleAddPastedItems = () => { /* ... unchanged ... */ setBulkAddMessage(null); const lines = bulkAddText.split('\n'); const itemNames = lines.map(line => line.trim()).filter(name => name !== ''); if (itemNames.length === 0) { setBulkAddMessage({ type: 'error', text: 'No valid item names found.' }); return; } if (currentTotalItemsCount + itemNames.length > MAX_ITEMS_CONFIG) { setBulkAddMessage({ type: 'error', text: `Cannot add. Total items would exceed limit of ${MAX_ITEMS_CONFIG}.` }); return; } dispatch(addMultipleItemEntries({ names: itemNames })); setBulkAddMessage({ type: 'success', text: `${itemNames.length} items added!` }); setBulkAddText(''); setTimeout(() => setBulkAddMessage(null), 4000); };

    const openPointerColorModal = () => {
        setTemporaryPointerColor(currentCustomPointerColor); // Initialize modal with current stored color
        setIsPointerColorModalOpen(true);
    };

    const handleApplyPointerColor = () => {
        dispatch(setCustomPointerColor(temporaryPointerColor));
        setIsPointerColorModalOpen(false);
    };

    const handleCancelPointerColor = () => {
        setIsPointerColorModalOpen(false);
        // temporaryPointerColor will be reset by the useEffect when modal closes if currentCustomPointerColor hasn't changed
    };
                        ...
                        <section className="p-4 bg-slate-700/60 rounded-lg shadow">
                            ...
                            <div className="mt-6 pt-4 border-t border-slate-600/50">
                                ...
                                <div className="flex items-center space-x-3">
                                    <div
                                        className="w-10 h-8 rounded-md border-2 border-slate-500 shadow-sm cursor-pointer hover:border-sky-400 transition-colors"
                                        style={{ backgroundColor: currentCustomPointerColor }}
                                        onClick={!isDisabled ? openPointerColorModal : undefined}
                                        title={`Current pointer color: ${currentCustomPointerColor}. Click to change.`}
                                        role="button"
                                        tabIndex={isDisabled ? -1 : 0}
                                        onKeyDown={!isDisabled ? (e) => { if (e.key === 'Enter' || e.key === ' ') openPointerColorModal(); } : undefined}
                                    ></div>
                                    <span className="text-sm text-slate-200 font-mono tabular-nums">{currentCustomPointerColor}</span>
                                    <button
                                        type="button"
                                        onClick={openPointerColorModal}
                                        disabled={isDisabled}
                                        className="ml-auto px-3 py-1.5 text-xs bg-sky-600 hover:bg-sky-700 text-white rounded-md focus:outline-none focus-visible:ring-2 focus-visible:ring-sky-500 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-700/60 disabled:opacity-50 disabled:cursor-not-allowed"
                                    >
                                        Choose Color...
                                    </button>
                                </div>
                                <p className="mt-1 text-xs text-slate-400">Set the main fill color for the wheel pointer.</p>
                            </div>
                            {/* ***** END MODIFIED POINTER COLOR INPUT ***** */}
                        </section>
                    </div>
                )}

                {/* Other tabs (Effects, Odds, History) - unchanged */}
                {activeTab === 'effects' && ( <div id="effects-tab-panel" className="p-3 sm:p-4 h-full"> <EffectsConfigurationPanel isDisabled={isDisabled} /> </div>)}
                {activeTab === 'odds' && ( <div id="odds-tab-panel" className="p-3 sm:p-4 h-full"> <OddsDisplayPanel /> </div>)}
                {activeTab === 'history' && ( <div id="history-tab-panel" className="p-3 sm:p-4 h-full"> <HistoryPanel /> </div>)}
            </div>

            {/* Pointer Color Picker Modal */}
            {isPointerColorModalOpen && (
                <Modal
                    isOpen={isPointerColorModalOpen}
                    onClose={handleCancelPointerColor} // Default close is cancel
                    title="Select Pointer Color"
                    size="sm" // Keep it compact for a color picker
                    footerContent={
                        <>
                            <button
                                onClick={handleCancelPointerColor}
                            ...
                            <button
                                onClick={handleApplyPointerColor}
                                className="px-4 py-2 text-sm font-medium text-white bg-sky-600 hover:bg-sky-700 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2 focus:ring-offset-slate-800 transition-colors"
                            >
                                Apply Color
                            </button>
                        </>
                ...
                >
                    <div className="space-y-4 p-1 sm:p-2"> {/* Padding inside modal body */}
                        <div className="mx-auto w-full max-w-[200px] sm:max-w-[240px] h-auto aspect-square rounded-md overflow-hidden shadow-lg border border-slate-600">
                            <HexColorPicker
                                color={temporaryPointerColor}
                                onChange={setTemporaryPointerColor}
                                style={{ width: '100%', height: '100%' }} // Make picker fill its container
                            />
                        </div>
                        <div className="flex items-center space-x-2 pt-2">
                            <HexColorInput
                                ...
                                onChange={setTemporaryPointerColor}
                                prefixed
                                alpha={false} // Pointer fill is solid
                                className="flex-grow w-full px-3 py-2 bg-slate-700 border border-slate-500 text-slate-100 rounded-md shadow-sm focus:outline-none focus:ring-1 focus:border-sky-500 focus:ring-sky-500 text-sm"
                            />
                            <div
                                className="w-8 h-8 rounded border-2 border-slate-500 flex-shrink-0 shadow-inner" // Added shadow-inner
                                style={{ backgroundColor: temporaryPointerColor }}
                                title={`Preview: ${temporaryPointerColor}`}
                            ></div>
                        </div>
                    </div>
                </Modal>
