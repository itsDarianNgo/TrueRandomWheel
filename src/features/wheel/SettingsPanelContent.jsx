...
import { selectAllItems, selectCategoryOdds } from '../items/itemSlice'; // Added selectCategoryOdds

// Icons (same as Response #45)
...
// UrlInputGroup (from ImageInputControl) is now part of ImageInputControl.jsx
...
const SettingsPanelContent = ({ onShuffleNTimes }) => {
    ...
    const allItems = useSelector(selectAllItems); // Used for itemsCount and total in odds display
    ...
    const categoryOdds = useSelector(selectCategoryOdds); // New selector for category odds
    ...
    // Handlers (handlePointerPositionChange, etc. same as Response #45)
    ...
    const handleSegmentOpacityChange = (e) => dispatch(setSegmentOpacity(parseInt(e.target.value, 10) / 100));
    ...
    return (
        <div className={`flex flex-col h-full ${isDisabled ? 'opacity-70 pointer-events-none' : ''}`}>
            {/* Tab Navigation (same as Response #45) */}
            ...
            <div className="flex-grow overflow-y-auto scrollbar-hide bg-slate-800">
                {activeTab === 'settings' && (
                    <div id="settings-tab-panel" role="tabpanel" aria-labelledby="settings-tab" className="p-3 sm:p-4 space-y-6">
                        {/* Pointer Position, Spin Behavior, Shuffle Wheel Sections (same as Response #45) */}
                        ...
                        {/* ***** NEW Category Odds Section ***** */}
                        {itemsCount > 0 && categoryOdds.length > 0 && (
                            <section className="p-4 bg-slate-700/60 rounded-lg shadow">
                                <h4 className="text-base sm:text-md font-semibold text-sky-300 mb-3 text-center">Category Odds</h4>
                                <ul className="space-y-1">
                                    {categoryOdds.map((catData) => {
                                        const displayName = catData.categoryName === null ? "Uncategorized" : catData.categoryName;
                                        const percentage = (catData.odds * 100).toFixed(1);
                                        return (
                                            <li key={displayName} className="flex justify-between items-center text-sm py-1.5 px-2 hover:bg-slate-600/50 rounded-md">
                                                <span className="flex items-center space-x-2">
                                                    <span
                                                        className="w-3 h-3 rounded-sm border border-slate-500 flex-shrink-0"
                                                        style={{ backgroundColor: catData.representativeColor || '#718096' }} // slate-500 as fallback
                                                        title={catData.representativeColor ? `Rep. color: ${catData.representativeColor}` : "Default color"}
                                                    ></span>
                                                    <span className="text-slate-200 truncate" title={displayName}>{displayName}</span>
                                                </span>
                                                <span
                                                    className="text-sky-300 font-semibold tabular-nums"
                                                    title={`(${catData.totalQuantity} / ${itemsCount} items)`}
                                                >
                                                    {percentage}%
                                                </span>
                                            </li>
                        ...
                        {itemsCount > 0 && categoryOdds.length === 0 && (
                            <section className="p-4 bg-slate-700/60 rounded-lg shadow text-center">
                                <p className="text-sm text-slate-400 italic">No items have categories assigned.</p>
                            </section>
                        )}
                        {itemsCount === 0 && (
                            <section className="p-4 bg-slate-700/60 rounded-lg shadow text-center">
                                <p className="text-sm text-slate-400 italic">Add items to the wheel to see category odds.</p>
                            </section>
                        )}
                        {/* ***** END Category Odds Section ***** */}
                    </div>
                )}
                {activeTab === 'appearance' && ( /* ... Appearance tab content same as Response #45 ... */ <div id="appearance-tab-panel" role="tabpanel" aria-labelledby="appearance-tab" className="p-3 sm:p-4 space-y-6"> <section className="p-4 bg-slate-700/60 rounded-lg shadow space-y-4"> <h4 className="text-md font-semibold text-sky-300 mb-1 text-center">Page & Wheel Images</h4> <ImageInputControl label="Page Background Image" idPrefix="page-bg" currentImageUrlFromStore={pageBgUrlFromStore} onSetImageUrl={(url) => dispatch(setPageBackgroundImageUrl(url))} disabled={isDisabled} /> <hr className="border-slate-600 my-4"/> <ImageInputControl label="Wheel Surface Image" idPrefix="wheel-surface" currentImageUrlFromStore={wheelSurfaceUrlFromStore} onSetImageUrl={(url) => dispatch(setWheelSurfaceImageUrl(url))} disabled={isDisabled} /> </section> <section className="p-4 bg-slate-700/60 rounded-lg shadow"> <h4 className="text-md font-semibold text-sky-300 mb-3 text-center">Wheel Appearance</h4> <div> <label htmlFor="segmentOpacity" className="block text-sm font-medium text-slate-300 mb-1"> Segment Color Opacity (on Wheel Image): <span className="font-semibold text-sky-300 ml-1">{Math.round(segmentOpacityFromStore * 100)}%</span> </label> <input type="range" id="segmentOpacity" min="0" max="100" value={Math.round(segmentOpacityFromStore * 100)} onChange={handleSegmentOpacityChange} disabled={isDisabled || !wheelSurfaceUrlFromStore} title={!wheelSurfaceUrlFromStore ? "Set a wheel surface image to enable opacity control" : `Current opacity: ${Math.round(segmentOpacityFromStore * 100)}%`} className={`w-full h-2 bg-slate-600 rounded-lg appearance-none cursor-pointer accent-sky-500 ${ (isDisabled || !wheelSurfaceUrlFromStore) ? 'opacity-50 cursor-not-allowed' : ''}`} /> <p className="mt-1 text-xs text-slate-400">Controls how much item colors cover the wheel surface image. Only active if a wheel surface image is set.</p> </div> </section> </div> )}
