...
// ... (imports, icons, etc. mostly same as Response #10 or #12)
...
import {
    ...
    setPointerPosition,
    toggleRemoveOnHit,
    setShuffleCount as setReduxShuffleCount,
    // performShuffleThunk is dispatched by WheelPage via onShuffleNTimes prop
} from './wheelSlice';
import { selectAllItems } from '../items/itemSlice';

// ... (Icon components and constants)
...
const SettingsPanelContent = ({ onShuffleNTimes }) => { // onShuffleNTimes is still passed from WheelPage
    ...
    useEffect(() => { // Sync local input with Redux state if it changes (e.g. on load)
    ...
    const handleShuffleCountInputChange = (e) => {
        ...
        setLocalShuffleCountInput(val); // Update local input immediately
        const count = parseInt(val, 10);
        if (!isNaN(count) && count >= 1) {
            dispatch(setReduxShuffleCount(count));
        } else if (val === "") {
            // If user clears input, Redux might default to 1 or last valid.
            // For consistency, let's ensure Redux gets a valid number or a defined behavior for empty.
            dispatch(setReduxShuffleCount(1)); // Default to 1 if input is cleared or invalid
        }
    };

    return ( /* ... JSX for tabs and content is the same as Response #10 / #12 ... */
        /* Key parts:
           - Pointer buttons dispatch(setPointerPosition(pos_key))
           - Remove on hit toggle onClick={handleRemoveOnHitChange} aria-checked={wheelSettings.removeOnHit}
           - Shuffle count input value={localShuffleCountInput} onChange={handleShuffleCountInputChange}
           - Shuffle N Times button onClick={onShuffleNTimes} (this prop calls the thunk in WheelPage)
        */
        <div className={`flex flex-col h-full ${isDisabled ? 'opacity-70 pointer-events-none' : ''}`}>
            ...
            <div className="flex-grow overflow-y-auto scrollbar-hide bg-slate-800">
                ...
                {activeTab === 'history' && ( <div id="history-tab-panel" role="tabpanel" aria-labelledby="history-tab" className="h-full"> <HistoryPanel /> </div> )}
            </div>
        </div>
    );
};
