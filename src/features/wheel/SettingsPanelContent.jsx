...
const SettingsPanelContent = ({ onShuffleNTimes }) => {
    ...
    const handleShuffleCountInputChange = (e) => { /* ... same as Response #30 ... */ const val = e.target.value; setLocalShuffleCountInput(val); const count = parseInt(val, 10); if (!isNaN(count) && count >= 1) { dispatch(setReduxShuffleCount(count)); } else if (val === "") { dispatch(setReduxShuffleCount(1)); } };
    ...
    return (
        <div className={`flex flex-col h-full ${isDisabled ? 'opacity-70 pointer-events-none' : ''}`}>
            {/* Tab Navigation (same as Response #30) */}
                ...
                {activeTab === 'appearance' && (
                    <div id="appearance-tab-panel" role="tabpanel" aria-labelledby="appearance-tab" className="p-3 sm:p-4 space-y-6">
                        <section className="p-4 bg-slate-700/60 rounded-lg shadow space-y-4">
                            <h4 className="text-md font-semibold text-sky-300 mb-1 text-center">Page & Wheel Images</h4>
                            <ImageInputControl
                                label="Page Background Image"
                                idPrefix="page-bg"
                                currentImageUrlFromStore={pageBgUrlFromStore}
                            ...
                            <ImageInputControl
                                ...
                                idPrefix="wheel-surface"
                                currentImageUrlFromStore={wheelSurfaceUrlFromStore}
                        ...
                        <section className="p-4 bg-slate-700/60 rounded-lg shadow">
                            ...
                            <div>
                                <label htmlFor="segmentOpacity" className="block text-sm font-medium text-slate-300 mb-1">
                                    Segment Color Opacity (on Wheel Image):
                                    <span className="font-semibold text-sky-300 ml-1">{Math.round(segmentOpacityFromStore * 100)}%</span>
                                </label>
                                <input
                                    type="range"
                                    id="segmentOpacity"
                                    ...
                                    title={!wheelSurfaceUrlFromStore ? "Set a wheel surface image to enable opacity control" : `Current opacity: ${Math.round(segmentOpacityFromStore * 100)}%`}
                                ...
                                />
                                <p className="mt-1 text-xs text-slate-400">Controls how much item colors cover the wheel surface image. Only active if a wheel surface image is set.</p>
                            </div>
                        </section>
                    </div>
                )}
                {activeTab === 'history' && ( /* ... History tab content same as Response #30 ... */ <div id="history-tab-panel" role="tabpanel" aria-labelledby="history-tab" className="h-full"> <HistoryPanel /> </div> )}
            </div>
        </div>
    );
};
