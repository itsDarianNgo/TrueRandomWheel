...
// ... (other imports remain the same from Response #45/34)
...
const WheelPage = () => {
    // ... (dispatch, selectors for items, wheelStatus, etc. - same as Response #34)
    ...
    const PANEL_WIDTH_PX = 480; const WHEEL_SECTION_TRANSLATE_X_OFFSET = PANEL_WIDTH_PX / 2.5; const WHEEL_SCALE_WHEN_PANEL_OPEN = 0.75; const overallIsBusy = wheelStatus !== 'idle';
    useEffect(() => { /* ... canvas resize observer same ... */ const wheelElement = wheelAreaRef.current; if (!wheelElement) return; const resizeObserver = new ResizeObserver(entries => { for (let entry of entries) { const { width, height } = entry.contentRect; const size = Math.min(width, height); if (size > 0 && (Math.abs(canvasDimensions.width - size) > 1 || Math.abs(canvasDimensions.height - size) > 1)) { setCanvasDimensions({ width: size, height: size }); } } }); resizeObserver.observe(wheelElement); return () => resizeObserver.unobserve(wheelElement); }, [canvasDimensions.width, canvasDimensions.height]);


    // MODIFIED Effect to apply page background image
    useEffect(() => {
        ...
        if (pageBackgroundImageUrl && /^(https?:\/\/|data:image\/)/i.test(pageBackgroundImageUrl)) {
            ...
            // Ensure no solid background color from this element itself obscures the image
            pageElement.style.backgroundColor = 'transparent';
        } else {
            // Clear inline styles to allow CSS class background (from App.jsx) to show
            pageElement.style.backgroundImage = '';
            pageElement.style.backgroundSize = '';
            pageElement.style.backgroundPosition = '';
            pageElement.style.backgroundRepeat = '';
            pageElement.style.backgroundColor = ''; // Also clear this
        }
    }, [pageBackgroundImageUrl]);

    // ... (Callbacks handleConfirmWin, handleVoidSpin, handleQuickShuffle, handleShuffleNTimes, toggleSettingsPanel - same as Response #34)
    ...
    return (
        // MODIFIED: Removed explicit bg-slate-850 from this div.
        // The default background (e.g., gradient from App.jsx) will now show through
        // if no pageBackgroundImageUrl is set by the useEffect.
        <div
            ref={pageRootRef}
            className="w-full flex flex-col items-center justify-start p-4 sm:p-6 lg:p-8 space-y-8 min-h-screen relative overflow-x-hidden transition-all duration-500 ease-in-out"
        >
            {/* Settings Toggle Button (same as Response #34) */}
            ...
            {/* Winning Banner Modal (same structure and logic as Response #34) */}
            ...
            <div className="w-full mx-auto flex flex-col items-center justify-center flex-grow relative">
                <section className="flex flex-col items-center space-y-4 transition-transform duration-300 ease-in-out w-full h-full flex-grow justify-center" style={{ transform: `translateX(${wheelTranslateXVal}px)` }}>
                    {/* MODIFIED DIV for background cutout fix (from Response #30) */}
                    <div
                        ref={wheelAreaRef}
                        ...
                        // This was the fix for wheel area container, already present.
                        // The issue now is page-wide background.
                    >
                        {/* ... (WheelCanvasContainer and empty state JSX remains same) ... */}
            ...
            {/* SettingsPanelOrchestrator (same props as Response #30) */}
            ...
            {/* Debug Bar (same JSX as Response #30) */}
