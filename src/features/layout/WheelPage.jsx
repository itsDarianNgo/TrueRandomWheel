...
// ... (other imports: itemSlice, wheelSlice actions/selectors, components - same as Response #45)
...
import WheelOddsSummary from '../odds/WheelOddsSummary'; // New import
...
const WheelPage = () => {
    // ... (dispatch, selectors, local state, refs - same as Response #45)
    ...
    const wheelTranslateXVal = isSettingsPanelOpen ? -WHEEL_SECTION_TRANSLATE_X_OFFSET : 0; // Only applied if panel is open
    ...
    return (
        <div ref={pageRootRef} className="w-full flex flex-col items-center justify-start p-2 sm:p-4 md:p-6 space-y-4 md:space-y-6 min-h-screen relative overflow-x-hidden transition-all duration-500 ease-in-out">
            ...
            {winningItemDetails && displayWinningBanner && ( <Modal isOpen={displayWinningBanner} onClose={handleConfirmWin} title="Spin Result!" size="md" panelClassName="bg-gradient-to-br from-purple-600 via-pink-600 to-red-600 text-white border-pink-500 shadow-2xl animate-[spin-celebrate_0.5s_ease-out]" footerContent={ <div className="flex flex-col sm:flex-row justify-end space-y-2 sm:space-y-0 sm:space-x-3 w-full"> <button onClick={handleVoidSpin} className="w-full sm:w-auto order-2 sm:order-1 px-4 py-2.5 text-sm font-semibold text-white bg-red-500 hover:bg-red-600 active:bg-red-700 rounded-lg shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-red-400 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-800 transition-colors" > Void Spin </button> <button onClick={handleConfirmWin} className="w-full sm:w-auto order-1 sm:order-2 px-4 py-2.5 text-sm font-semibold text-slate-900 bg-green-400 hover:bg-green-500 active:bg-green-600 rounded-lg shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-green-300 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-800 transition-colors" > Confirm Winner </button> </div> } > <div className="text-center py-4"> <p className="text-3xl md:text-5xl font-bold">🎉 Winner! 🎉</p> <p className="text-xl md:text-3xl mt-4"> Landed on: <span className="font-extrabold tracking-wide text-yellow-300">{winningItemDetails.name}</span> </p> {removeOnHitActive && ( <p className="mt-3 text-xs text-white/80 px-2"> (Confirming will remove this item from the wheel as "Remove on Hit" is active) </p> )} {!removeOnHitActive && ( <p className="mt-3 text-xs text-white/80 px-2"> (Item will remain on the wheel) </p> )} </div> </Modal> )}

            {/* MODIFIED Main Content Area for Wheel and Odds Summary */}
            <div className={`w-full flex-grow flex flex-col md:flex-row items-center md:items-start md:justify-center gap-4 lg:gap-6 transition-transform duration-300 ease-in-out`}
                 style={{ transform: isSettingsPanelOpen ? `translateX(${wheelTranslateXVal}px)` : 'translateX(0px)' }} // Apply translate only if panel is open
            >
                {/* Wheel Area Column */}
                <section className="flex-grow w-full md:w-auto flex justify-center items-center order-1 md:order-1 min-w-0 md:min-w-[320px] lg:min-w-[400px] xl:min-w-[480px]">
                    {/* This div handles the bg-slate-800 or bg-transparent for wheel area */}
                    <div
                        ref={wheelAreaRef}
                        className={`p-3 sm:p-4 rounded-xl shadow-2xl flex items-center justify-center w-full max-w-full md:max-w-[800px] h-auto aspect-square relative 
                ...
                {/* Odds Summary Column (only shown if not opening settings panel, or always visible based on layout choice) */}
                {/* For MVP, keep it always next to wheel unless panel is open pushing it away. */}
                <section className="w-full md:w-auto md:max-w-xs lg:max-w-sm order-2 md:order-2 shrink-0 mt-4 md:mt-0">
                    <WheelOddsSummary onViewDetailedOddsClick={toggleSettingsPanel} />
                </section>
            </div>


            {/* Settings Panel Orchestrator (same) */}
