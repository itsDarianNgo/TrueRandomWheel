...
// ... (other imports remain the same as Response #66)
...
const WheelPage = () => {
    // ... (state, refs, constants, effects, callbacks - same as Response #66)
    const dispatch = useDispatch(); const items = useSelector(selectAllItems); const wheelStatus = useSelector(selectWheelStatus); const winningItemDetails = useSelector(selectWinningItemDetails); const displayWinningBanner = useSelector(selectDisplayWinningBanner); const pageBackgroundImageUrl = useSelector(selectPageBackgroundImageUrl); const removeOnHitActive = useSelector(selectRemoveOnHit); const [isSettingsPanelOpen, setIsSettingsPanelOpen] = useState(false); const [canvasDimensions, setCanvasDimensions] = useState({ width: 520, height: 520 }); const wheelAreaRef = useRef(null); const pageRootRef = useRef(null); const PANEL_WIDTH_PX = 480; const WHEEL_SECTION_TRANSLATE_X_OFFSET = PANEL_WIDTH_PX / 2.5; const WHEEL_SCALE_WHEN_PANEL_OPEN = 0.75; const overallIsBusy = wheelStatus !== 'idle';
    useEffect(() => { const wheelElement = wheelAreaRef.current; if (!wheelElement) return; const resizeObserver = new ResizeObserver(entries => { for (let entry of entries) { const { width, height } = entry.contentRect; const size = Math.min(width, height); if (size > 0 && (Math.abs(canvasDimensions.width - size) > 1 || Math.abs(canvasDimensions.height - size) > 1)) { setCanvasDimensions({ width: size, height: size }); } } }); resizeObserver.observe(wheelElement); return () => resizeObserver.unobserve(wheelElement); }, [canvasDimensions.width, canvasDimensions.height]);
    useEffect(() => { const pageElement = pageRootRef.current; if (!pageElement) return; if (pageBackgroundImageUrl && /^(https?:\/\/|data:image\/)/i.test(pageBackgroundImageUrl)) { pageElement.style.backgroundImage = `url('${pageBackgroundImageUrl}')`; pageElement.style.backgroundSize = 'cover'; pageElement.style.backgroundPosition = 'center'; pageElement.style.backgroundRepeat = 'no-repeat'; pageElement.style.backgroundColor = 'transparent'; } else { pageElement.style.backgroundImage = ''; pageElement.style.backgroundSize = ''; pageElement.style.backgroundPosition = ''; pageElement.style.backgroundRepeat = ''; pageElement.style.backgroundColor = ''; } }, [pageBackgroundImageUrl]);
    ...
    return (
        <div ref={pageRootRef} className="w-full flex flex-col items-center justify-start p-2 sm:p-4 md:p-6 space-y-4 md:space-y-6 min-h-screen relative overflow-x-hidden transition-all duration-500 ease-in-out">
            ...
            {/* Main Content Area for Wheel and Odds Summary */}
            <div
                className={`w-full flex-grow flex flex-col md:flex-row items-center md:items-start md:justify-center gap-4 lg:gap-6 transition-transform duration-300 ease-in-out`}
                style={{ transform: isSettingsPanelOpen ? `translateX(${wheelTranslateXVal}px)` : 'translateX(0px)' }}
            >
                {/* Wheel Area Column */}
                {/* MODIFIED: Let this column also grow and try to use height */}
                <section className="w-full md:flex-1 flex flex-col justify-center items-center order-1 md:order-1 min-w-0 p-1 md:p-2 self-stretch"> {/* md:flex-1 and self-stretch */}
                    <div
                        ref={wheelAreaRef}
                        className={`p-1 sm:p-2 rounded-xl shadow-2xl flex items-center justify-center w-full h-full max-w-full aspect-square relative 
                                    transition-colors duration-300 ${pageBackgroundImageUrl && /^(https?:\/\/|data:image\/)/i.test(pageBackgroundImageUrl) ? 'bg-transparent' : 'bg-slate-800'}`}
                        // Removed md:max-w-[800px] and explicit max-h to allow it to grow more,
                        // aspect-square on h-full w-full within a flex item will make it fit the smaller dimension of its allocated space.
                        // The parent 'section' with md:flex-1 will compete for space with the odds summary.
                    >
                        <div
                            className="relative origin-center transition-transform duration-300 ease-in-out w-full h-full" // w-full h-full for content
                            style={{
                        ...
                        >
                            ...
                            />
                            {items.length === 0 && !overallIsBusy && ( /* Empty state */ <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-800/90 backdrop-blur-sm rounded-full pointer-events-none p-5"> <IconWheelOutlineDisplay /> <p className="text-lg font-medium text-slate-300 mt-3 text-center">Your wheel is empty!</p> <p className="text-sm text-slate-400 text-center">Use the settings panel to add items.</p> </div>)}
                        </div>
                        <button onClick={handleQuickShuffle} disabled={overallIsBusy || items.length < 2} /* Quick shuffle button */ className="absolute top-3 right-3 sm:top-4 sm:right-4 z-10 p-2.5 bg-slate-700/80 hover:bg-slate-600/90 text-slate-100 rounded-full shadow-lg focus:outline-none focus-visible:ring-2 focus-visible:ring-sky-400 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-800 transition-all duration-150 ease-in-out hover:scale-110 active:scale-100 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100" aria-label="Quick shuffle wheel items" title="Shuffle Items (Once)"> <IconShuffle className="w-5 h-5" /> </button>
                    </div>
                </section>

                {/* Odds Summary Column */}
                <section className="w-full md:w-auto md:max-w-xs lg:max-w-sm order-2 md:order-2 shrink-0 mt-4 md:mt-0 self-center md:self-start"> {/* self-center on mobile, self-start on md+ */}
                    <WheelOddsSummary onViewDetailedOddsClick={toggleSettingsPanel} />
                </section>
            </div>

            {/* Settings Panel Orchestrator & Debug Bar (same) */}
