...
// ... (imports and top part of component same as Response #69/user's code) ...
...
const WheelPage = () => {
    ...
    const WHEEL_TRANSLATE_X_WHEN_PANEL_OPEN = -(PANEL_WIDTH_PX / 2.5); const WHEEL_SCALE_WHEN_PANEL_OPEN = 0.75; const overallIsBusy = wheelStatus !== 'idle';


    // MODIFIED useEffect for canvas resizing
    useEffect(() => {
        const currentWheelArea = wheelAreaRef.current; // Capture ref value
        if (!currentWheelArea) {
            // console.log('[WheelPage_ResizeEffect] wheelAreaRef.current is null. Bailing.');
            return;
        }

        // console.log('[WheelPage_ResizeEffect] Initializing ResizeObserver.');
        const resizeObserver = new ResizeObserver(entries => {
            for (let entry of entries) {
                const { width: observedWidth, height: observedHeight } = entry.contentRect;
                // console.log(`[WheelPage_ResizeEffect] Observed wheelAreaRef: W=${observedWidth}, H=${observedHeight}`);

                const newSize = Math.floor(Math.min(observedWidth, observedHeight));
                // console.log(`[WheelPage_ResizeEffect] Calculated newSize (min of W,H): ${newSize}`);

                // Update canvasDimensions only if newSize is positive and significantly different
                // Access previous state via functional update to avoid stale closure on canvasDimensions
                setCanvasDimensions(prevDimensions => {
                    // console.log(`[WheelPage_ResizeEffect] Inside setCanvasDimensions. Prev: W=${prevDimensions.width}, H=${prevDimensions.height}. NewSize: ${newSize}`);
                    if (newSize > 0 && (prevDimensions.width !== newSize || prevDimensions.height !== newSize)) {
                        // console.log(`[WheelPage_ResizeEffect] SETTING canvasDimensions to: ${newSize}x${newSize}`);
                        return { width: newSize, height: newSize };
                    }
                    // console.log('[WheelPage_ResizeEffect] No change to canvasDimensions.');
                    return prevDimensions; // No change
                });
            }
        });
        ...
        return () => {
            // console.log('[WheelPage_ResizeEffect] Cleaning up ResizeObserver.');
            if (currentWheelArea) { // Check if ref is still valid on cleanup
                resizeObserver.unobserve(currentWheelArea);
            }
        };
        // Dependency array: Run effect when component mounts and if wheelAreaRef instance changes (should not happen often for a stable ref).
        // Removing canvasDimensions from here, as the update logic is inside the callback.
    }, []); // Empty dependency array for mount/unmount behavior for the observer itself.

    useEffect(() => { /* ... page background effect (same as Response #69) ... */ const pageElement = pageRootRef.current; if (!pageElement) return; if (pageBackgroundImageUrl && /^(https?:\/\/|data:image\/)/i.test(pageBackgroundImageUrl)) { pageElement.style.backgroundImage = `url('${pageBackgroundImageUrl}')`; pageElement.style.backgroundSize = 'cover'; pageElement.style.backgroundPosition = 'center'; pageElement.style.backgroundRepeat = 'no-repeat'; pageElement.style.backgroundColor = 'transparent'; } else { pageElement.style.backgroundImage = ''; pageElement.style.backgroundSize = ''; pageElement.style.backgroundPosition = ''; pageElement.style.backgroundRepeat = ''; pageElement.style.backgroundColor = ''; } }, [pageBackgroundImageUrl]);
    ...
    // console.log('[WheelPage] Rendering. canvasDimensions passed to Container:', canvasDimensions);

    return ( /* ... JSX structure identical to Response #69 ... */
        <div ref={pageRootRef} className="w-full flex flex-col items-center justify-start p-2 sm:p-4 md:p-6 space-y-4 md:space-y-6 min-h-screen relative overflow-x-hidden transition-all duration-500 ease-in-out">
            ...
            {winningItemDetails && displayWinningBanner && ( <Modal isOpen={displayWinningBanner} onClose={handleConfirmWin} title="Spin Result!" size="md" panelClassName="bg-gradient-to-br from-purple-600 via-pink-600 to-red-600 text-white border-pink-500 shadow-2xl animate-[spin-celebrate_0.5s_ease-out]" footerContent={ <div className="flex flex-col sm:flex-row justify-end space-y-2 sm:space-y-0 sm:space-x-3 w-full"> <button onClick={handleVoidSpin} className="w-full sm:w-auto order-2 sm:order-1 px-4 py-2.5 text-sm font-semibold text-white bg-red-500 hover:bg-red-600 active:bg-red-700 rounded-lg shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-red-400 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-800 transition-colors" > Void Spin </button> <button onClick={handleConfirmWin} className="w-full sm:w-auto order-1 sm:order-2 px-4 py-2.5 text-sm font-semibold text-slate-900 bg-green-400 hover:bg-green-500 active:bg-green-600 rounded-lg shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-green-300 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-800 transition-colors" > Confirm Winner </button> </div> } > <div className="text-center py-4 flex flex-col items-center"> <p className="text-3xl md:text-5xl font-bold">🎉 Winner! 🎉</p> <p className="text-xl md:text-3xl mt-4"> Landed on: <span className="font-extrabold tracking-wide text-yellow-300">{winningItemDetails.name}</span> </p> {removeOnHitActive && ( <p className="mt-3 text-xs text-white/80 px-2"> (Confirming will remove this item from the wheel as "Remove on Hit" is active) </p> )} {!removeOnHitActive && ( <p className="mt-3 text-xs text-white/80 px-2"> (Item will remain on the wheel) </p> )} </div> </Modal> )}
            <div className={`w-full flex-grow flex flex-col md:flex-row items-center md:items-stretch md:justify-center gap-4 lg:gap-6 transition-transform duration-300 ease-in-out`} style={{ transform: `translateX(${wheelTranslateXVal}px)`}} >
                <section className="w-full md:flex-[3] lg:flex-[4] flex flex-col justify-center items-center order-1 md:order-1 min-w-0 p-1 md:p-0 self-stretch">
                    <div ref={wheelAreaRef} className={ `p-1 sm:p-2 rounded-xl shadow-xl flex items-center justify-center w-full h-full aspect-square relative transition-colors duration-300 ${pageBackgroundImageUrl && /^(https?:\/\/|data:image\/)/i.test(pageBackgroundImageUrl) ? 'bg-transparent' : 'bg-slate-800'}` } >
                        <div className="relative origin-center transition-transform duration-300 ease-in-out w-full h-full flex justify-center items-center" style={{ transform: `scale(${visualWheelScale})`}} >
                            <WheelCanvasContainer width={canvasDimensions.width} height={canvasDimensions.height} />
                        </div>
                        {items.length === 0 && !overallIsBusy && ( <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-800/90 backdrop-blur-sm rounded-full pointer-events-none p-5 z-10"> <IconWheelOutlineDisplay /> <p className="text-lg font-medium text-slate-300 mt-3 text-center">Your wheel is empty!</p> <p className="text-sm text-slate-400 text-center">Use the settings panel to add items.</p> </div>)}
                        <button onClick={handleQuickShuffle} disabled={overallIsBusy || items.length < 2} className="absolute top-3 right-3 sm:top-4 sm:right-4 z-20 p-2.5 bg-slate-700/80 hover:bg-slate-600/90 text-slate-100 rounded-full shadow-lg focus:outline-none focus-visible:ring-2 focus-visible:ring-sky-400 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-800 transition-all duration-150 ease-in-out hover:scale-110 active:scale-100 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100" aria-label="Quick shuffle wheel items" title="Shuffle Items (Once)"> <IconShuffle className="w-5 h-5" /> </button>
                    </div>
                </section>
