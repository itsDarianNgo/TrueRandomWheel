...
const itemSlice = createSlice({
    ...
    reducers: {
        ...
        // ***** NEW REDUCER for updating an item group *****
        updateItemGroup: (state, action) => {
            const { sourceGroupId, newName, newColor, newTags } = action.payload;
            // console.log(`[itemSlice] updateItemGroup for ${sourceGroupId}:`, action.payload);
            state.items = state.items.map(item => {
                if (item.sourceGroup === sourceGroupId) {
                    return {
                        ...item,
                        name: newName,
                        color: newColor === undefined ? undefined : (newColor || undefined), // Ensure empty string for color becomes undefined
                        tags: newTags || []
                    };
                }
                return item;
...
export const {
    ...
    removeItemInstance,
    updateItemGroup // Export new action
} = itemSlice.actions;
